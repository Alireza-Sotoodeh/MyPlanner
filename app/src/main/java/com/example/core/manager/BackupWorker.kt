...
class BackupWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        if (!backupFileManager.hasWritePermission(rootUri)) {
            ...
            backupFileManager.prefs.edit().remove("backup_location_uri").apply()
            return@withContext Result.retry()
        }
            ...
            try {
                ...
                var permanentDir = backupFileManager.findChildUri(rootUri, "_permanent")
                if (permanentDir == null) {
                    permanentDir = backupFileManager.getOrCreateDir(rootUri, "_permanent")
                }
                if (permanentDir != null) {
                ...
                var monthDir = backupFileManager.findChildUri(rootUri, currentMonth)
                if (monthDir == null) {
                    monthDir = backupFileManager.getOrCreateDir(rootUri, currentMonth)
                }
                if (monthDir != null) {
                ...
                backupFileManager.prefs.edit().putLong("drive_last_sync_at", System.currentTimeMillis()).apply()
